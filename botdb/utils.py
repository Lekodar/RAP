...
def gen_random():
    return randint(1111111, 9999999)
...
async def start_handle(message: types.Message):
    ...
    await message.answer('Привет!\nЭто чат бот по онлайн рапортичке\nВсе функции можно узнать по команде /help')


@dp.message(Command('help'))
async def handle(message: types.Message):
    await message.answer('''
/addstud – Подключить БД (SQLite) и при нажатии нужно будет ввести ФИО, кол опозданий, кол пропусков. Далее записывает в БД полученные данные)
/list – Берёт из БД все ФИО и выводит их
/del – удалить из БД студента
/rep – Берет данные из БД, создает файл Excel, и записывает в этот файл данные из БД. И отправляет этот файл
/mark ФИО, потом бот запросит опоздал или пропустил и записывает в Бд
''')
